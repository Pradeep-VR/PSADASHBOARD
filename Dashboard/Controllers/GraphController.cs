            ...
            try
            {
                if (groupId != "")
                {
                    ...
                    Qry = "WITH TIMEINTERVALS AS ( SELECT CONVERT(DATETIME, ( SELECT TOP 1 SYNCDATETIME  FROM TBL_ENERGYMETER E   WHERE CONVERT(DATETIME, E.SYNCDATETIME, 126) BETWEEN CONVERT(DATETIME, '" + formattedStartDate + "', 126) " +
                            ...
                            "TBL_ENERGYMETER E  ON CAST(CONVERT(DATETIME, E.SYNCDATETIME, 126) AS SMALLDATETIME) = CAST(T.INTERVALTIME AS SMALLDATETIME)  LEFT JOIN  METERMASTER M ON E.METERID = M.METERID    WHERE E.GROUPID = '" + groupId.Trim() + "' AND M.METERDIVISION IN (" + Divison + ")) SELECT   DATETIMES," +
